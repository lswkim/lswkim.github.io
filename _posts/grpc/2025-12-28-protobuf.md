...
flowchart LR
    A["int32 user_id = 1;"]
    B["타입<br/>(int32, string, bool 등)"]
    C["필드 이름<br/>(코드에서 사용)"]
    D["필드 번호<br/>(바이너리 인코딩에 사용)"]
...
```mermaid
flowchart TB
    subgraph Problem["실제 운영 상황"]
        A["Day 1: v1 서버, v1 클라이언트 배포"]
        B["Day 30: 요구사항 변경!<br/>User에 phone 필드 추가 필요"]
        C["문제: 서버는 v2, 클라이언트는 아직 v1<br/>모바일 앱은 업데이트에 시간 걸림"]
        D["→ v1 클라이언트와 v2 서버가 통신 가능해야 함!"]
    end
