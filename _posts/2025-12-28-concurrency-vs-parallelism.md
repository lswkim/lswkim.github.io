---
title: "[FastAPI 기초개념 2] 동시성(Concurrency) vs 병렬성(Parallelism)"
...
```mermaid
gantt
    title 4코어 CPU, 2개 프로세스 → 코어가 남음 (비효율)
    dateFormat X
    axisFormat %s
    section Core 1
    P1 :0, 4
...
```mermaid
gantt
    title 4코어 CPU, 4개 프로세스 → 모든 코어 활용 (최적)
    dateFormat X
    axisFormat %s
    section Core 1
    P1 :0, 4
...
```mermaid
gantt
    title 4코어 CPU, 8개 프로세스 → 컨텍스트 스위칭 (오버헤드)
    dateFormat X
    axisFormat %s
    section Core 1
    P1 :0, 1
