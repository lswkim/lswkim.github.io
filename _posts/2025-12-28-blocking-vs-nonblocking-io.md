---
title: "[FastAPI 기초개념 3] 블로킹(Blocking) vs 논블로킹(Non-blocking) I/O"
...
sequenceDiagram
    ...
    participant 직원
    나->>직원: 김치찌개 주세요
    Note over 나: 카운터 앞에서 대기...
    Note over 나: (5분 경과)
    직원-->>나: 김치찌개 나왔습니다
    Note over 나: 음식 받고 자리로 이동
```
...
sequenceDiagram
    ...
    participant 직원
    나->>직원: 김치찌개 주세요
    직원-->>나: 진동벨 드릴게요 (번호 42번)
    Note over 나: 자리에서 다른 일<br/>(핸드폰, 대화 등)
    직원->>나: (5분 후) 진동벨 울림
    Note over 나: 음식 받으러 감
```
...
sequenceDiagram
    ...
    App->>Kernel: 소켓 A, B, C, D 중<br/>읽을 준비 된 거 알려줘
    Note over Kernel: 소켓 A: 데이터 없음<br/>소켓 B: 데이터 도착!<br/>소켓 C: 데이터 없음<br/>소켓 D: 데이터 도착!
    Kernel-->>App: B, D 준비됐어!
